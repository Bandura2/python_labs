...
class SetManager:
    ...
    def __len__(self):
        return sum(len(insect.favorite_food_set) for insect in self.regular_manager)

    def __getitem__(self, index__):
        for insect in self.regular_manager:
            if index__ < len(insect.favorite_food_set):
                return list(insect.favorite_food_set)[index__]
            index__ -= len(insect.favorite_food_set)
        raise IndexError
    ...
    def __next__(self):
        ...
        list_set = list(insect.favorite_food_set)
